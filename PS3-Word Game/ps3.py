...
def get_word_score(word, n):
    ...
    if n >=len(L):
        sumscore1=0
        for i in word:
            sumscore1 += SCRABBLE_LETTER_VALUES[i]
        sumscore2=(7* wordlen-3*(n- wordlen))   
        if  sumscore2<1:
            sumscore2=1
...
def is_valid_word(word, hand, word_list):
    ...
    for i in word:
        if i=='*':
            for j in VOWELS:
                word0=word[:count]+j+word[(count+1):]
                if word0 in word_list:
                    break
                else:
                    if j=='u': #last vowel
                        return False
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    for i in range(n):
        ...
        print("Current Hand:")
        display_hand(hand)
        ans1=input("Would you like to substitute a letter? ")
        ans1=ans1.lower()
        if ans1=="yes":
